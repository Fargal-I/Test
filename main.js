...
if (brain) {
	brain.addEventListener("click", function (e) {
		result++;
		popupTitle.textContent = 'Шахматы, олимпиады по математике, программирование и прочее.Судя по всему этому, я довольно умён. Но переваливать себя нельзя,расслабляюсь ещё 😅'
	});
}
...
if (came) {
	came.addEventListener("click", function (e) {
		result++;
		popupTitle.textContent = 'Повезло-повезло)'
	});
}
...
if (notCame) {
	notCame.addEventListener("click", function (e) {
		result++;
		popupTitle.textContent = '1 из 3 моих друзей - однокурсников не пришли, так что это тоже верно)'
	});
}
